...
export default function BentoCard() {
    ...
    return (
        <Card className="w-full max-w-sm bg-[#131313] border-white/5 shadow-2xl overflow-hidden">
            <CardHeader className="pb-4">
                <CardTitle className="text-white text-lg">Generate scores based on pictures</CardTitle>
            ...
            <CardContent>
                <div className="grid grid-cols-2 gap-3 relative">
                    {candidates.map((candidate, index) => {
                        ...
                        return (
                            // The wrapper now has padding (p-3) which creates the gap between the blue frame and the image
                            <div
                                key={candidate.id}
                                className="relative aspect-[3/4] p-3 cursor-pointer group"
                                onClick={() => setActiveIndex(index)}
                            >
                                {/* Image Container - securely nested inside the padding */}
                                <div className="w-full h-full relative rounded-lg overflow-hidden bg-zinc-900 shadow-inner">
                                    <motion.img
                                        ...
                                        animate={{
                                            ...
                                            scale: isActive ? 1.1 : 1, // Sweeter zoom
                                ...
                                {/* The "Scanner" Frame moving between inset-0 of the padded cells */}
                                {isActive && (
                                    <motion.div
                                        layoutId="scanner-frame"
                                    ...
                                    >
                                        {/* Viewfinder Corners with a subtle glow (blur) to mimic the scanner vibe */}
                                        ...
                                        <AnimatePresence>
                                            <motion.div
                                                ...
                                                // bottom-5 carefully places it right over the image's bottom border
                                                className="absolute bottom-5 left-1/2 -translate-x-1/2 bg-white text-black px-2 py-0.5 rounded-md text-[11px] flex items-center gap-1.5 shadow-xl"
                                            >
