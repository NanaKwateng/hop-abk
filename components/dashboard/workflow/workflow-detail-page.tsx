...
export function WorkflowDetailPage({ workflow }: WorkflowDetailPageProps) {
    ...
    const filteredMembers = useMemo(() => {
        if (!memberSearch.trim()) return workflow.members;
        const q = memberSearch.toLowerCase();
        return workflow.members.filter(
            (m) =>
                m.firstName.toLowerCase().includes(q) ||
                m.lastName.toLowerCase().includes(q) ||
                m.membershipId?.toLowerCase().includes(q)
        );
    }, [workflow.members, memberSearch]);
    ...
    const memberEntryCounts = useMemo(() => {
        ...
        entries.forEach((e) =>
            counts.set(e.memberId, (counts.get(e.memberId) ?? 0) + 1)
        );
        return counts;
    }, [entries]);
    ...
    const memberTotals = useMemo(() => {
        ...
        entries.forEach((e) => {
            if (e.amount != null)
                totals.set(
                    e.memberId,
                    (totals.get(e.memberId) ?? 0) + e.amount
                );
        });
        return totals;
    ...
    const handleDrawerEntryChange = useCallback(
        (newEntries: WorkflowEntry[]) => {
            setEntries((prev) => {
                ...
                return [...newEntries, ...otherEntries].sort(
                    (a, b) =>
    ...
    async function handleDeleteEntry() {
        ...
        startTransition(async () => {
            try {
                ...
                setEntries((prev) =>
                    prev.filter((e) => e.id !== deleteEntryId)
                );
    ...
    // ═══════════════════════════════════════════════════════
    // EXPORT UTILITY (Adapts your export-utils logic)
    // ═══════════════════════════════════════════════════════
    const handleExport = useCallback((formatType: "csv" | "excel") => {
        // Define export columns based dynamically on workflow type
        const columns = [
            ...
            <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6">
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                            <span className="flex items-center gap-1">
                                ...
                                {format(new Date(workflow.startDate), "MMM d")} –{" "}
                                {format(
                                    new Date(workflow.endDate),
                                    "MMM d, yyyy"
                                )}
                            </span>
                            <span className="flex items-center gap-1">
                                <Users className="h-3.5 w-3.5" />
                                {workflow.memberCount} members
                            </span>
                            ...
                            <div>
                                ...
                                <p className="text-lg font-bold tabular-nums">
                                    {workflow.type === "payments"
                                        ...
                                        : `${processedMemberIds.size}/${workflow.memberCount}`}
                                </p>
                            </div>
                        </CardContent>
                    </Card>
                ...
                {isExpired && (
                    <Alert variant="destructive">
                        ...
                        <AlertDescription>
                            This workflow&apos;s end date has passed. You can
                            still view and add entries.
                        </AlertDescription>
                    </Alert>
                )}
                    ...
                    <Card className="lg:col-span-1">
                        <CardHeader className="pb-3">
                            <CardTitle className="text-base flex items-center gap-2">
                                ...
                                Members ({workflow.members.length})
                            </CardTitle>
                            ...
                            <ScrollArea className="h-[500px]">
                                <div className="divide-y">
                                    {filteredMembers.map((member) => {
                                        const isProcessed =
                                            processedMemberIds.has(member.memberId);
                                        const isActive =
                                        ...
                                        const initials =
                                            ...
                                            (member.lastName?.[0] ?? "");
                                        const entryCount =
                                            memberEntryCounts.get(
                                                member.memberId
                                            ) ?? 0;
                                        const memberTotal =
                                            memberTotals.get(member.memberId) ?? 0;
                        ...
                        <Card>
                            ...
                            <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between pb-4">
                                <div className="space-y-1">
                                    ...
                                    <CardDescription>
                                        Processed {config.label.toLowerCase()} for
                                        this workflow.
                                    </CardDescription>
                                </div>
                                {entries.length > 0 && (
