...
// actions/member.ts (modified for offline support)
...
export async function fetchMembers(params: MemberQueryParams): Promise<PaginatedResponse<Member>> {
    ...
    return {
        data: rowsToMembers((data as MemberRow[]) || []),
        totalCount,
        totalPages,
...
export async function createMember(data: MemberFormData, avatarBase64?: string): Promise<Member> {
    return withActionRetry(async () => {
        ...
        try {
            ...
            await offlineDB.put('members', offlineMember);
            ...
            return rowToMember(offlineMember as unknown as MemberRow);
        } catch (offlineError) {
            console.error('[Offline] Failed to save member:', offlineError);
            throw new Error('Failed to save member offline. Please try again.');
        }
    });
}
...
export async function updateMember(id: string, data: Partial<MemberFormData>, avatarBase64?: string): Promise<Member> {
    return withActionRetry(async () => {
        ...
        try {
            ...
            // Return optimistic result
            const existingMember = await offlineDB.get('members', id);
            return rowToMember((existingMember || offlineMember) as unknown as MemberRow);
        } catch (offlineError) {
            console.error('[Offline] Failed to update member:', offlineError);
            throw new Error('Failed to update member offline. Please try again.');
        }
    });
}
...
export async function deleteMember(id: string): Promise<void> {
    return withActionRetry(async () => {
        ...
        try {
            ...
            await offlineDB.delete('members', id);
            ...
            console.error('[Offline] Failed to delete member:', offlineError);
            throw new Error('Failed to delete member offline. Please try again.');
        }
    });
}
...
export async function deleteMultipleMembers(ids: string[]): Promise<void> {
    return withActionRetry(async () => {
        ...
        if (isOnline) {
            ...
            if (members) {
                for (const m of members) {
                    if (m.avatar_url) await deleteAvatarByUrl(m.avatar_url);
                }
            }
        ...
        try {
            ...
            console.error('[Offline] Failed to delete members:', offlineError);
            throw new Error('Failed to delete members offline. Please try again.');
        }
    });
}
...
export async function duplicateMember(id: string): Promise<Member> {
    return withActionRetry(async () => {
        ...
        // If online, duplicate directly in Supabase
        if (isOnline) {
            const { data: duplicate, error: insertError } = await supabase
                .from("members")
        ...
        try {
            ...
            console.error('[Offline] Failed to duplicate member:', offlineError);
            throw new Error('Failed to duplicate member offline. Please try again.');
        }
    });
}