...
function mapWorkflow(w: any): Workflow {
    return {
        id: w.id, slug: w.slug, name: w.name, type: w.type,
...
function mapEntry(e: any): WorkflowEntry {
    return {
        id: e.id, workflowId: e.workflow_id, memberId: e.member_id,
        ...
        roleTitle: e.role_title, roleDescription: e.role_description,
        entryType: e.entry_type ?? "record",
        paymentDate: e.payment_date, status: e.status ?? "pending",
...
export async function getWorkflows(): Promise<Workflow[]> {
    ...
    const { data, error } = await supabase
        .from("workflows")
        .select(`id, slug, name, type, start_date, end_date, created_by, created_at, updated_at, workflow_members(count)`)
...
export async function getWorkflowBySlug(slug: string): Promise<WorkflowDetail | null> {
    ...
    const { data: workflow, error: wfError } = await supabase
        .from("workflows").select("*").eq("slug", cleanSlug).single();
    if (wfError || !workflow) return null;

    const { data: workflowMembers, error: mErr } = await supabase
        .from("workflow_members")
        .select(`id, workflow_id, member_id, created_at, members!inner(id, first_name, last_name, membership_id, avatar_url, member_group)`)
    ...
    const { data: entries, error: eErr } = await supabase
        ...
        .select(`id, workflow_id, member_id, title, description, amount, role_title, role_description, entry_type, payment_date, status, created_by, created_at, updated_at, members!inner(first_name, last_name, avatar_url)`)
        .eq("workflow_id", workflow.id)
        .order("created_at", { ascending: false });
    if (eErr) throw new Error(eErr.message);

    const members = (workflowMembers ?? []).map((wm: any) => ({
        id: wm.id, workflowId: wm.workflow_id, memberId: wm.member_id,
        firstName: wm.members.first_name, lastName: wm.members.last_name,
        membershipId: wm.members.membership_id, avatarUrl: wm.members.avatar_url,
        memberGroup: wm.members.member_group, createdAt: wm.created_at,
    }));

    return {
        id: workflow.id, slug: workflow.slug, name: workflow.name, type: workflow.type,
        startDate: workflow.start_date, endDate: workflow.end_date,
        memberCount: members.length, createdBy: workflow.created_by,
...
// ── NEW: Add members to existing workflow ──
...
// ── NEW: Remove members from existing workflow ──
export async function removeWorkflowMembers(
    ...
    return withActionRetry(async () => {
        ...
        if (entryError) {
            console.error("[removeWorkflowMembers] Entry cleanup failed:", entryError);
        }
...
export async function createWorkflowEntry(payload: CreateWorkflowEntryPayload): Promise<WorkflowEntry> {
    return withActionRetry(async () => {
        ...
        const { data: entry, error } = await supabase.from("workflow_entries").insert({
            workflow_id: payload.workflowId, member_id: payload.memberId,
            title: payload.title, description: payload.description || null,
            amount: payload.amount ?? null, role_title: payload.roleTitle || null,
            role_description: payload.roleDescription || null,
            entry_type: payload.entryType, payment_date: payload.paymentDate || null,
            status: payload.status || "completed",
            created_by: user.id,
        }).select(`id, workflow_id, member_id, title, description, amount, role_title, role_description, entry_type, payment_date, status, created_by, created_at, updated_at, members!inner(first_name, last_name, avatar_url)`).single();
        if (error) throw new Error(error.message);
        revalidatePath("/admin/workflows");
...
export async function updateWorkflowEntry(payload: UpdateWorkflowEntryPayload): Promise<WorkflowEntry> {
    return withActionRetry(async () => {
        ...
        const { data: entry, error } = await supabase.from("workflow_entries").update(updateData)
            .eq("id", payload.id).eq("workflow_id", payload.workflowId)
            .select(`id, workflow_id, member_id, title, description, amount, role_title, role_description, entry_type, payment_date, status, created_by, created_at, updated_at, members!inner(first_name, last_name, avatar_url)`).single();
        if (error) throw new Error(error.message);
        revalidatePath("/admin/workflows");
        return mapEntry(entry);
